                             ...
                             int printflags, int iflag, int iter, int nfev) )
{
    ...
    float *fvec, *diag, *fjac, *qtf, *wa1, *wa2, *wa3, *wa4;
    int *ipvt;
    
    int n = n_par;
    int m = m_dat;
    
    if ( (fvec = (float *) malloc(m * sizeof(float))) == NULL ||
        (diag = (float *) malloc(n * sizeof(float))) == NULL ||
        (qtf  = (float *) malloc(n * sizeof(float))) == NULL ||
        (fjac = (float *) malloc(n*m*sizeof(float))) == NULL ||
        (wa1  = (float *) malloc(n * sizeof(float))) == NULL ||
        (wa2  = (float *) malloc(n * sizeof(float))) == NULL ||
        (wa3  = (float *) malloc(n * sizeof(float))) == NULL ||
        (wa4  = (float *) malloc(m * sizeof(float))) == NULL ||
...
void lm_lmpar( int n, float *r, int ldr, int *ipvt, float *diag,
              ...
              float *sdiag, float *aux, float *xdi );
              ...
              int printflags, const void *data )
{
     ...
     *
     *        void evaluate( float* par, int m_dat, void *data,
     *                       float* fvec, int *info )
     *        {
     *           // for ( i=0; i<m_dat; ++i )
...
float lm_enorm(int n, const float *x)
{
